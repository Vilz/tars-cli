...
````bash
# Будет создана страница inner.{html, pug} на основе _template.{html, pug}
tars add-page inner
...
* `--exclude-html`: файл _template.{html,hbs,pug} обновляется по умолчанию. Такое поведение можно отменить, используя данный флаг.
